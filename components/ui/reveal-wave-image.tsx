...
function ImagePlane({
  ...
  // "cover" scale: fill the entire canvas like CSS object-fit: cover
  // The OrthographicCamera-like setup: plane is 2x2 units, camera z=1
  // Visible world area = 2 high × (2 * canvasAspect) wide
  const scale = useMemo<[number, number, number]>(() => {
    const canvasAspect = size.width / size.height;
    const imgAspect = aspectRatio;

    let sx: number, sy: number;
    if (canvasAspect > imgAspect) {
      // Canvas is wider relative to image → clamp to canvas width
      sx = canvasAspect;
      sy = canvasAspect / imgAspect;
    } else {
      // Canvas is taller relative to image → clamp to canvas height
      sx = imgAspect;
      sy = 1;
    }
    return [sx, sy, 1];
